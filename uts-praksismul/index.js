...
hueInput.onchange = (e) => {
  hueText = `hue-rotate(${(e.target.value / 10) * 36 - 180}deg)`;
  updateImageFilter();
};
saturationInput.onchange = (e) => {
  saturationText = `saturate(${e.target.value / 100})`;
  updateImageFilter();
};
...
const updateImageFilter = () => {
  compressedImage.style.filter = `${grayscaleText} ${hueText} ${saturationText} ${brightnessText} ${contrastText} ${blurText}`;
};
